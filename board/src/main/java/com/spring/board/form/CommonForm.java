...
public class CommonForm {
	...
	int current_page_no; // 현재 페이지 번호
	int count_per_page; // 한화면에 출력될 페이지 수
	int count_per_list; // 한 화면에 출력될 게시글 수
	int tatal_page_count; // 총 페이지 수 
	int tatal_list_count; // 총 게시글 수 
	int limit; // 가져올 row의 수
	int offset; // 몇번 째 row 부터 가져올 지를 결정
